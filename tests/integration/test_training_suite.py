...
def test_cancel_via_cloud_api_endpoint():
    ...
    mock_firestore.get_job.return_value = {
        "task_id": "test_task_123",
        "status": "RUNNING",
        "progress": {"message": "Training in progress"}
    }
    mock_firestore.update_job_status.return_value = True

    # Create a job in the training service first
    training_service = importlib.import_module('services.training_service')
    req = make_request_obj()
    task_id, kwargs = training_service.create_job(req)
    ...
    # Don't mock cancel_job - let it use the real implementation
    with patch('src.api.v1.endpoints.cloud.get_firestore_client', return_value=mock_firestore):

        app = FastAPI()
        app.include_router(cloud_router_module.router)
        client = TestClient(app)

        # Test successful cancellation
        resp = client.delete(f"/cloud/jobs/{task_id}")
        assert resp.status_code == 200
        body = resp.json()
        ...
        assert body["pipeline_stopped"] is True  # Should be True since we set up a running job
        assert "message" in body

        # Verify Firestore was called
        mock_firestore.update_job_status.assert_called_once()
        update_call = mock_firestore.update_job_status.call_args
        assert update_call[0][0] == task_id  # task_id
        assert str(update_call[0][1]) == "JobStatus.CANCELLED"  # status
...
def test_cancel_via_cloud_api_endpoint_pipeline_stop_failure():
    ...
    # Mock training service cancel_job to return False (pipeline stop failed)
    with patch('src.api.v1.endpoints.cloud.get_firestore_client', return_value=mock_firestore), \
         patch('src.services.training_service.cancel_job') as mock_cancel_job:
        ...
        # Test cancellation with pipeline stop failure
        resp = client.delete("/cloud/jobs/test_task_456")
        assert resp.status_code == 200
        body = resp.json()

        # Verify response indicates pipeline stop failure
        assert body["task_id"] == "test_task_456"
        assert body["status"] == "cancelled"
        assert body["pipeline_stopped"] is False
...
def test_cancel_via_cloud_api_endpoint_firestore_failure():
    ...
    mock_firestore.update_job_status.return_value = False
    ...
    with patch('src.api.v1.endpoints.cloud.get_firestore_client', return_value=mock_firestore), \
        ...
        # Test cancellation with Firestore failure
        resp = client.delete("/cloud/jobs/test_task_789")
        assert resp.status_code == 500
        body = resp.json()
        assert "Failed to cancel job" in body["detail"]
...
def test_training_cancellation_with_interrupted_error():
    ...
    with patch('torch.optim.AdamW'), \
         ...
         patch('src.train.training.nn.TripletMarginLoss'), \
         patch('src.train.wandb_logger.wandb_logger') as mock_wandb:
...
def test_pipeline_cancellation_with_pending_stop():
    ...
    from unittest.mock import patch, MagicMock
    from src.common.pipeline import Pipeline, stop_pipeline

    # Mock pipeline components
    with patch('src.common.google_storage.get_storage') as mock_storage, \
         patch('src.common.google_storage.GCSPaths') as mock_paths:
