...
import numpy as np
from cv2 import (COLOR_BGR2RGB, COLOR_RGB2BGR, COLOR_RGB2HSV, VideoCapture,
                 cvtColor, inRange)
...
class BackgroundMaskDetector:
    ...
    def __init__(self, config: Optional["BackgroundMaskConfig"] = None, **kwargs):
        """
    ...
    def _detect_background_color(self, frame_generator: Generator[np.ndarray, None, None]):
        ...
        # Ensure values are not None for type checking
        assert self.mean_hsv is not None, "mean_hsv should be calculated"
        assert self.std_hsv is not None, "std_hsv should be calculated"

        # Calculate bounds
        self.lower_bound = np.maximum(
            self.mean_hsv - self.std_dev_multiplier * self.std_hsv, self.hsv_min_values
        ).astype(np.uint8)
        self.upper_bound = np.minimum(
            self.mean_hsv + self.std_dev_multiplier * self.std_hsv, self.hsv_max_values
        ).astype(np.uint8)
    ...
    def remove_background(
        self, images: Union[np.ndarray, List[np.ndarray]], input_format: str = "BGR"
    ...
    def update_bounds(self, new_std_multiplier: float):
        ...
        if new_std_multiplier < self.min_std_multiplier:
            if self.verbose:
                print(
                    f"Warning: std_multiplier {new_std_multiplier} is below minimum "
                    f"{self.min_std_multiplier}. Using minimum."
                )
            new_std_multiplier = self.min_std_multiplier
        elif new_std_multiplier > self.max_std_multiplier:
            if self.verbose:
                print(
                    f"Warning: std_multiplier {new_std_multiplier} is above maximum "
                    f"{self.max_std_multiplier}. Using maximum."
                )
...
def create_frame_generator_from_images(
    images: List[np.ndarray], input_format: str = "RGB"
...
if __name__ == "__main__":
    ...
    print("Test completed. Background detection and removal working correctly.")
    ...
    print("Custom config test completed with blue replacement color.")
    ...
    print("Parameter override test completed with yellow replacement color.")
