...
This module provides functional approaches to crop extraction, reorganization,
...
def extract_crops_from_video(
    ...
    for frame in frame_generator:
        ...
        crops_extracted_this_frame = _extract_crops_from_frame(
            frame, detections, frame_idx, all_crops_dir, crop_sizes, crop_widths, crop_heights
        )
...
def _extract_crops_from_frame(
    ...
    for i in range(len(detections)):
        ...
        if (
            ...
            and "frame_id" in detections.data
            and len(detections.data["frame_id"]) > i
        ):
...
def _save_crop(
    crop_rgb: np.ndarray, tracker_id: int, frame_id: int, confidence: float, all_crops_dir: str
) -> bool:
    """
...
def _log_crop_statistics(
    crop_sizes: List[int], crop_widths: List[int], crop_heights: List[int]
) -> None:
...
def reorganize_crops_by_stitched_tracks(
    all_crops_dir: str, temp_dir: str, track_id_mapping: dict[int, int]
) -> None:
    """
    ...
    """
    logger.info("Reorganizing crops based on stitched tracks mapping")
    ...
    logger.info("Crop reorganization complete!")
    logger.info(f"Processed {processed_original_tracks} original track directories")
    logger.info(
        f"Moved {total_crops_moved} crops to {len(stitched_track_ids)} stitched track directories"
    )
...
def create_train_val_split(
    source_folder: str, destin_folder: str, train_ratio: float = 0.8
) -> None:
    """
    ...
    has_frame_folders = any(
        item.startswith("frame_") and os.path.isdir(os.path.join(source_folder, item))
        for item in items_in_source
    )
    has_crop_folders = any(
        item.startswith("crop_") and os.path.isdir(os.path.join(source_folder, item))
        for item in items_in_source
    )
    ...
    for tracker_id, crop_list in tracker_crops.items():
        ...
        for source_folder_name, crop_file in train_crops:
            if has_crop_folders:
                ...
                # Old structure: source_folder/frame_123/123_456_0.900.jpg
                # -> train/456/123_456_0.900.jpg
                src = os.path.join(source_folder, source_folder_name, crop_file)
        ...
        for source_folder_name, crop_file in val_crops:
            if has_crop_folders:
                ...
                # Old structure: source_folder/frame_123/123_456_0.900.jpg
                # -> val/456/123_456_0.900.jpg
                src = os.path.join(source_folder, source_folder_name, crop_file)
            dst = os.path.join(val_track_dir, crop_file)
            shutil.copy2(src, dst)

    logger.info(
        f"Crops split with {train_ratio*100:.0f}% for training and "
        f"{100 - train_ratio*100:.0f}% for validation."
    )
...
class CropExtractor:
    ...
    def extract_crops(self) -> None:
        ...
        self.crops_dir, self.all_crops_dir = extract_crops_from_video(
            self.frame_generator, self.all_detections, self.temp_dir, self.crop_extract_interval
        )
