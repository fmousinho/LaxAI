    ...
    def _load_paths(self) -> dict:
        ...
        try:
            ...
            if self.gcs_paths_file.startswith("/config/"):
                ...
                if not os.path.isabs(self.gcs_paths_file):
                    # Get the project root (two levels up from shared_libs/common/)
                    current_file_path = os.path.abspath(__file__)
                    project_root = os.path.dirname(os.path.dirname(os.path.dirname(current_file_path)))
                    actual_path = os.path.join(project_root, self.gcs_paths_file)
                else:
                    actual_path = self.gcs_paths_file
            ...
            logger.error(
                f"GCS paths configuration file not found: "
                f"{actual_path if 'actual_path' in locals() else self.gcs_paths_file}"
            )
            raise
...
def ensure_ready(func):
    ...
    def wrapper(self, *args, **kwargs):
        if not self._ensure_authenticated():
            ...
            if (
                func.__name__.startswith(("download", "get"))
                or func.__name__ == "download_as_appropriate_type"
                or func.__name__ == "download_as_string"
            ):
                return None
...
class GoogleStorageClient:
        ...
        try:
            ...
            if self.credentials:
                self._client = storage.Client(
                    credentials=self.credentials, project=self.config.project_id
                )
                logger.info(
                    f"Using provided service account credentials for project: "
            ...
            logger.info(
                f"Successfully authenticated with Google Cloud Storage for bucket: "
                f"{self.config.bucket_name}"
            )
            return True
            ...
            logger.error(
                "Set GOOGLE_APPLICATION_CREDENTIALS for local use, or ensure a "
                "service account is attached in GCP."
            )
            return False
        except NotFound as e:
            logger.error(
                f"Authentication failed - Bucket '{self.config.bucket_name}' not found: {e}"
            )
            return False
        except Forbidden as e:
            logger.error(
                f"Authentication failed - Access denied to bucket '{self.config.bucket_name}': {e}"
            )
            return False
        except Exception as e:
    ...
    def list_blobs(
        ...
        exclude_prefix_in_return: bool = False,
    ) -> set[str]:
        """
    ...
    def upload_from_bytes(
        self, destination_blob_name: str, data: bytes, content_type: Optional[str] = None
    ) -> bool:
        """
    ...
    def move_blob(self, source_blob_name: str, destination_blob_name: str) -> bool:
        ...
        try:
            ...
            self._bucket.copy_blob(source_blob, self._bucket, new_name=destination_blob_name)  # type: ignore
    ...
    class GCSVideoCapture:
        ...
        def __enter__(self):
            ...
            try:
                ...
                self._finalizer = weakref.finalize(
                    self, self._cleanup_temp_file, self.temp_file_path
                )
                ...
                if not self.cap.isOpened():
                    raise IOError(
                        f"Error: Could not open video file from GCS blob {self.gcs_blob.name}"
                    )
                return self
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            ...
            if self.temp_file_path and os.path.exists(self.temp_file_path):
                os.remove(self.temp_file_path)
                logger.debug(
                    f"Context manager cleaned up temporary video file: {self.temp_file_path}"
                )
