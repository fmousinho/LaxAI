...
from .gpu_memory import (clear_gpu_memory, get_gpu_memory_stats,
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Perform aggressive cleanup including tensor search"
    )
    parser.add_argument(
        "--stats-only", action="store_true", help="Only show memory statistics without cleanup"
    )
    ...
    print(
        f"  Peak:      {stats['max_allocated_gb']:.2f} GB ({stats['max_allocated_bytes']:,} bytes)"
    )
