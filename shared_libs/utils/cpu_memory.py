...
class CPUMemoryMonitor:
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "increase_percent": ((current["rss_mb"] - baseline["rss_mb"]) / baseline["rss_mb"]) * 100 if baseline["rss_mb"] > 0 else 0
        }
    ...
    def log_memory_stats(self, context: str = ""):
        ...
        if stats["increase_mb"] > 0:
            log_msg += f" | Increase: +{stats['increase_mb']:.1f}MB ({stats['increase_percent']:.1f}%)"
...
def clear_cpu_memory(force: bool = False):
    ...
    try:
        # This helps with numpy array cleanup
        import numpy as np
        np._cleanup()
...
def log_comprehensive_memory_stats(context: str = ""):
    ...
    if "process_rss_mb" in stats:
        log_msg += f"  Process: {stats['process_rss_mb']:.1f}MB RSS ({stats['process_percent']:.1f}%)\n"
    ...
    log_msg += f"  GC: {stats['gc']['gc_objects']} objects, {stats['gc']['gc_garbage_count']} garbage"
