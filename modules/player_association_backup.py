...
def predict_bbox_motion(track_data: TrackData, target_frame: int) -> Optional[np.ndarray]:
    """
    Predict bounding box position at target frame using simple linear motion model.
    
    Args:
        track_data: Track data containing historical bounding box positions
        target_frame: Frame number to predict position for
        
    Returns:
        Predicted bounding box [x1, y1, x2, y2] or None if prediction not possible
    """
    if len(track_data.crops) < 2:
        return None
    
    # Use the last few frames to estimate motion
    frame_span = min(5, len(track_data.crops))
    if frame_span < 2:
        return None
    
    # For simplicity, we'll use a basic linear extrapolation
    # In a real implementation, you'd track actual bbox positions
    # Here we'll return None to indicate motion prediction is not available
    # This can be enhanced with actual bbox tracking data
    return None
...
def calculate_stitching_cost(
    ...
    # Calculate motion cost (simplified - can be enhanced with actual motion prediction)
    motion_cost = 0.0
    predicted_bbox = predict_bbox_motion(track_a, track_b.frame_first_seen)
    if predicted_bbox is not None:
        # Motion prediction available - calculate distance
        # This is a placeholder - in practice you'd compare with track_b's starting position
        motion_cost = 0.1  # Placeholder cost
    else:
        # No motion prediction available
        motion_cost = 0.0
...
def create_representative_embedding(track_data: TrackData) -> np.ndarray:
    """
    Create a representative embedding for a tracklet by averaging all crop embeddings.
    
    This assumes the track_data.embedding already contains the averaged embedding
    from all crops in the tracklet. If individual crop embeddings are needed,
    this function would need to be enhanced.
    ...
    if track_data.embedding.size > 0:
        ...
        return np.zeros(model_config.embedding_dim)  # Use configured embedding dimension
...
def stitch_tracks(
    ...
    This function implements the track stitching algorithm described in the workflow:
    1. Generate representative embeddings for each tracklet
    2. Calculate matching costs between tracklets
    3. Perform greedy stitching based on lowest costs
    4. Update multi_frame_detections with new track IDs
    ...
    Returns:
        Tuple of (stitched tracks dictionary, updated multi_frame_detections)
    """
    ...
    for frame_detections in multi_frame_detections:
        if frame_detections.tracker_id is not None:
            ...
            if hasattr(updated_frame_detections, 'data') and updated_frame_detections.data:
                for key, values in updated_frame_detections.data.items():
                    ...
                    if 'track' in key.lower() and isinstance(values, (list, np.ndarray)):
                        ...
                        for value in values:
                            if isinstance(value, (int, np.integer)):
                                updated_values.append(track_id_mapping.get(value, value))
                            else:
                                updated_values.append(value)
                        updated_frame_detections.data[key] = np.array(updated_values) if isinstance(values, np.ndarray) else updated_values
                        data_updates_count += 1
                        logger.debug(f"Updated data field '{key}' with stitched track IDs")
                    
                    # Also check for any integer values that match known track IDs (more comprehensive)
                    elif isinstance(values, (list, np.ndarray)):
                        updated_values = []
                        field_updated = False
                        for value in values:
                            if isinstance(value, (int, np.integer)) and value in track_id_mapping:
                                updated_values.append(track_id_mapping[value])
                                field_updated = True
                            else:
                                updated_values.append(value)
                        if field_updated:
                    ...
                    elif isinstance(values, (int, np.integer)) and values in track_id_mapping:
                        updated_frame_detections.data[key] = track_id_mapping[values]
                        data_updates_count += 1
                        logger.debug(f"Updated data field '{key}' with stitched track ID (single value)")
            
            # Update any metadata that references tracker IDs
            if hasattr(updated_frame_detections, 'metadata') and updated_frame_detections.metadata:
                for key, value in updated_frame_detections.metadata.items():
                    if 'track' in key.lower():
                        if isinstance(value, (int, np.integer)):
                            updated_frame_detections.metadata[key] = track_id_mapping.get(value, value)
                            metadata_updates_count += 1
                            logger.debug(f"Updated metadata field '{key}' with stitched track ID")
                        elif isinstance(value, (list, np.ndarray)):
                            updated_values = [track_id_mapping.get(v, v) if isinstance(v, (int, np.integer)) else v for v in value]
                            updated_frame_detections.metadata[key] = np.array(updated_values) if isinstance(value, np.ndarray) else updated_values
                            metadata_updates_count += 1
                            logger.debug(f"Updated metadata field '{key}' with stitched track IDs")
                    
                    # Also check for any integer values that match known track IDs (more comprehensive)
                    elif isinstance(value, (int, np.integer)) and value in track_id_mapping:
                        updated_frame_detections.metadata[key] = track_id_mapping[value]
                        metadata_updates_count += 1
                        ...
                        field_updated = False
                        for v in value:
                            if isinstance(v, (int, np.integer)) and v in track_id_mapping:
                                updated_values.append(track_id_mapping[v])
                                field_updated = True
                            else:
    ...
    if metadata_updates_count > 0:
        logger.info(f"Updated {metadata_updates_count} metadata fields with stitched track IDs")
    if data_updates_count > 0:
        logger.info(f"Updated {data_updates_count} data fields with stitched track IDs")
    
    return stitched_tracks, updated_detections
...
def associate_tracks_to_players_with_stitching(
    ...
    This is the main function that implements the full track stitching workflow:
    1. Stitch fragmented tracklets together
    2. Associate the stitched tracks to players using global optimization
    3. Update multi_frame_detections with new track IDs
    ...
    stitched_tracks, updated_multi_frame_detections = stitch_tracks(
    ...
    for original_track_id in tracks_data.keys():
        # Find which stitched track this original track became part of
        found_player = None
...
def _track_is_part_of_stitched_track(original_track: TrackData, stitched_track: TrackData) -> bool:
    """
    Check if an original track is part of a stitched track by comparing frame ranges and team.
...
# Update the existing global association function to be the default
    """
    Associate tracks to players using global optimization to find the best overall assignment.
