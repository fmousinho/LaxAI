            ...
            for i in range(len(dataset)):
                try:
                    ...
                    if isinstance(image, torch.Tensor):
                        logger.debug(f"Image {i}: type=Tensor, shape={image.shape}")
                        logger.debug(f"  Image tensor stats: min={image.min():.4f}, max={image.max():.4f}, mean={image.mean():.4f}")
                    else:
                        logger.debug(f"Image {i}: type={type(image)}, converting to tensor...")
                    ...
                    logger.debug(f"Final embedding {i}: min={final_embedding.min():.4f}, max={final_embedding.max():.4f}, mean={final_embedding.mean():.4f}")
                    logger.debug(f"Final embedding norm: {torch.norm(final_embedding, p=2, dim=1).item():.4f}")
