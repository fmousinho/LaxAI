...
class ParameterRegistry:
    ...
    def generate_cli_parser_for_training(self, parser: Optional[argparse.ArgumentParser] = None) -> argparse.ArgumentParser:
    ...
    def generate_cli_parser_for_model(self, parser: Optional[argparse.ArgumentParser] = None) -> argparse.ArgumentParser:
    ...
    def get_config_value(self, param_name: str) -> Any:
        ...
        for part in config_path_parts[1:]:
            if hasattr(value, part):
                ...
                if param.required:
                    raise ValueError(f"Required parameter {param_name} not found in config at {param.config_path}")
                return None
    ...
    def get_kwarg_or_config(self, param_name: str, kwargs: Dict[str, Any]) -> Any:
        ...
        if param_def and param_def.required:
            raise ValueError(f"Missing required hyperparameter '{param_name}' in kwargs and config.")
        else:
            raise ValueError(f"Missing required hyperparameter '{param_name}' in kwargs and config.")
    
    def generate_cli_parser(self, parser: Optional[argparse.ArgumentParser] = None) -> argparse.ArgumentParser:
        ...
        for param in self.parameters.values():
            ...
            if param.type == ParameterType.FLOAT:
                ...
                kwargs['action'] = 'store_true' if not kwargs.get('default', False) else 'store_false'
    ...
    def _generate_cli_parser_for_params(self, params, parser: Optional[argparse.ArgumentParser] = None) -> argparse.ArgumentParser:
