...
def proxy():
    ...
    # Patch Firestore collection with shared DummyCollection instance
    dummy_collection = DummyCollection()
    proxy._progress_collection = MagicMock(spec=["document"])
    proxy._progress_collection.document.side_effect = dummy_collection.document
...
def test_start_tracking_job_persists_mapping(proxy):
    payload = {"task_id": "abc123", "foo": "bar", "tenant_id": "dummy-tenant", "custom_name": "test-job", "tracking_params": {"verbose": True}}
    ...
    assert doc._data["tenant_id"] == "dummy-tenant"
    assert doc._data["custom_name"] == "test-job"
    assert doc._data["tracking_params"] == {"verbose": True}
    # Verify payload field is not present (flattened)
    assert "payload" not in doc._data
