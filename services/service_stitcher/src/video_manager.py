...
class VideoManager:
    ...
    def get_players(self) -> Optional[List[Player]]:
        ...
        Returns:
            Optional[List[Player]]: List of Player objects if available, else None.
        """
        with self.lock:
    ...
    def update_player(self, player_id: int, name: Optional[str], tracker_ids: Optional[List[int]]) -> Optional[Player]:
        ...
        Args:
            player_id (int): Unique player ID to update
            name (Optional[str]): Updated name of the player
            tracker_ids (Optional[List[int]]): Updated list of tracker IDs associated with the player
        """
        with self.lock:
            if not self.player_manager:
                raise ValueError("Player manager is not initialized.")
    ...
    def delete_player(self, player_id: int) -> bool:
        """Delete a player from the player manager.

        Args:
            player_id (int): Unique player ID to delete 

        Returns:
            bool: True if the player was deleted successfully, False otherwise.
        """
        with self.lock:
            if not self.player_manager:
                raise ValueError("Player manager is not initialized.")
