...
class BYTETracker(object):
    ...
    def update(self, detections_array: np.ndarray, S_array: Optional[np.ndarray], T_array: Optional[np.ndarray]) -> List[STrack]:
        ...
        if len(remain_inds_mask) > 0:
            # Try to get frame dimensions from self._frame if set, otherwise use defaults
            img_h, img_w = 1080, 1920
            if self._frame is not None:
                img_h, img_w = self._frame.shape[:2]
            
            bboxes_remain = bboxes[remain_inds_mask]
            # A small margin helps ensure the detection is fully contained and not clipped
            margin = 2 
            not_touching_border = (
            ...
            remain_inds_mask = remain_inds_mask[not_touching_border]
       ...
       #  logger.debug(f"is_activated stracks before association: {len(tracked_stracks)} | pending conf: {len(pending_confirmation_tracks)} | lost: {len(self.lost_stracks)}")
        ...
        for itracked, idet in matches:
            ...
            self.update_detection_to_track_map(high_conf_mask[idet], track)
        ...
        self.step_log("step 1", strack_pool, matches, unmatched_tracks, u_detection, dists)

        ''' Step 2: Second association, with low score detection boxes'''
        if len(unmatched_tracks) > 0 and len(bboxes_low) > 0:
            dists = matching.v_iou_reid_distance(unmatched_tracks, bboxes_low, self.get_embeddings, iou_thresh=.3, reid_weight=.5)
            matches, u_track_second, u_detection_second = matching.linear_assignment(dists, thresh=.6) # thresh: cost
        else:
            matches = []
        ...
        for itracked, idet in matches:
            ...
            track.match_cost = dists[itracked, idet]
            self.update_detection_to_track_map(low_conf_mask[idet], track)
        ...
        self.step_log("step 2", unmatched_tracks, matches, u_track_second, u_detection_second, dists)
        ...
        """ Step 3: Scavenger for removed and unconfirmed tracks """

        """ Step 3.1: Verifieds if pending confirmation tracks could match older tracks """

        tracks_for_scavenging = self.removed_stracks + pending_confirmation_tracks
        scavenged_indices = set()
        
        if len(tracks_for_scavenging) > 0 and len(unmatched_bbox_array) > 0:
            # Use ReID for a more robust scavenger/confirmation with spatial gating
            dists = matching.v_iou_reid_distance(
                tracks_for_scavenging, 
                unmatched_detections_array, 
                self.get_embeddings, 
                iou_thresh=0.0, 
                reid_weight=1.0,
                max_spatial_distance=300,
                spatial_decay_frames=90,
                current_frame_id=self.frame_id
            )
            matches, u_confirmed, u_detection = matching.linear_assignment(dists, thresh=.1)
            for itracked, idet in matches:
                scavenged_indices.add(itracked)
                track = tracks_for_scavenging[itracked]
                bbox = unmatched_detections_array[idet, :4]
                score = unmatched_detections_array[idet, 4]
                track.match_cost = dists[itracked, idet]
                
                # Deals with removed tracks (track.state == Removed)
                track.update(STrack(STrack.tlbr_to_tlwh(bbox), score), self.frame_id)
                if track.state == TrackState.Removed:
                    track.mark_scavenged()
                    # Deals with unconfirmed tracks (track.state == Tracked, is_active == False)
                elif track.tracklet_len >= self.min_consecutive_frames:
                ...
                activated_stracks.append(track)
                self.update_detection_to_track_map(unmatched_detections_mask[idet], track)
        else:
            u_detection = np.arange(len(unmatched_detections_array))
            u_confirmed = np.arange(len(tracks_for_scavenging))
            matches = []
            dists = None

        self.step_log("step 3.1", tracks_for_scavenging, matches, u_confirmed, u_detection, dists)


        """ Step 3.2: Uses bytetracker to follow unconfirmed tracks """

        # Rebuilds the pending_confirmation_tracks list
        offset = len(self.removed_stracks)
        pending_confirmation_tracks = []
        for itrack in u_confirmed:
            track = tracks_for_scavenging[itrack]   
            if itrack < offset or itrack in scavenged_indices:                
                continue
            pending_confirmation_tracks.append(track)
        ...
        if len(pending_confirmation_tracks) > 0 and len(unmatched_detections_array) > 0:
            dists = matching.v_iou_distance(pending_confirmation_tracks, unmatched_detections_array)
            matches, u_confirmed, u_detection = matching.linear_assignment(dists, thresh=.8)
            ...
            u_detection = np.arange(len(unmatched_detections_array))
            u_confirmed = np.arange(len(pending_confirmation_tracks))
            matches = []
            dists = None

        for itrack in u_confirmed:
            track = pending_confirmation_tracks[itrack]
        ...
        self.step_log("step 3.2", pending_confirmation_tracks, matches, u_confirmed, u_detection, dists)

        """ Step 4: Create new tracks """
        for i, inew in enumerate(unmatched_detections_array):
            bbox = inew[:4]
            score = inew[4]
            if score < self.track_activation_threshold:
                continue
            ...
            self.update_detection_to_track_map(unmatched_detections_mask[i], track)
            n_tracks_new += 1

        self.step_log("step 4", None, None, None, None, None)
        ...
        self.step_log("step 5", None, None, None, None, None)

        # Update tracker state
        if len(scavenged_indices) > 0:
    ...
    def step_log(self, step, tracks, matches, unmatched_tracks, unmatched_detections, dists):
        if step == "step 1":
            logger.debug(f"=== Step 1: High Score Association ===")
            for itracked, idet in matches:
                track = tracks[itracked]
                dist = dists[itracked, idet]
                logger.debug(f"Track ID {track.track_id} matched to det {idet} with distance: {dist:.4f}")
        elif step == "step 2":
            logger.debug(f"=== Step 2: Low Score Association ===")
            for itracked, idet in matches:
                track = tracks[itracked]
                dist = dists[itracked, idet]
                logger.debug(f"Track ID {track.track_id} matched to det {idet} with distance: {dist:.4f}")
        elif step == "step 3.1":
            logger.debug(f"=== Step 3.1: ReID Scavenging ===")
            if dists is None:
                return
            # Log matched (scavenged) tracks
            if len(matches) > 0:
                logger.debug(f"=== ReID Matches ===")
                for itracked, idet in matches:
                    track = tracks[itracked]
                    dist = dists[itracked, idet]
            ...
            if len(unmatched_tracks) > 0:
                logger.debug(f"=== Unmatched ReID Tracks ===")
            ...
            logger.debug(f"=== Step 3.2: IoU Scavenging ===")
            if dists is None:
                return
            # Log matched (scavenged) tracks
            if len(matches) > 0:
                logger.debug(f"=== IoU Matches ===")
                for itracked, idet in matches:
                    track = tracks[itracked]
                    dist = dists[itracked, idet]
                    logger.debug(f"IoU: Track ID {track.track_id} matched to det {idet} with distance: {dist:.4f}")
            else:
                logger.debug(f"No IoU matches")
            # Log unmatched tracks with their best distances
            if len(unmatched_tracks) > 0:
                logger.debug(f"=== Unmatched IoU Tracks ===")
                for idx in unmatched_tracks:
                    track = tracks[idx]
                    t_dists = dists[idx]
                    min_dist = np.min(t_dists) if len(t_dists) > 0 else float('inf')
    ...
    def update_detection_to_track_map(self, det_position: np.ndarray, track: STrack):
        """
        Updates the detection to track mapping based on the current tracks.
        """
        confirmed_tracks_only = True
        if confirmed_tracks_only and not track.is_activated:
            return
        self._detection_to_track_map[det_position] = track.track_id
