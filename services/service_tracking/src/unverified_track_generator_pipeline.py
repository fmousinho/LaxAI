...
from shared_libs.utils.id_generator import create_run_id, create_video_id
    ...
    **Storage Organization:**
        ...
        try:
            ...
            with self.tenant_storage.get_video_capture(raw_video_path) as cap:
                ...
                if width_val is None or height_val is None:
                    raise RuntimeError(f"Could not read video dimensions from {raw_video_path}")
            ...
            video_folder_path = self.path_manager.get_path("imported_video", 
                                                    video_id=video_guid)
            if video_folder_path is None:
                raise RuntimeError(f"Failed to generate path for imported video with ID: {video_guid}")
            
            video_folder = video_folder_path.rstrip('/')
        ...
        try:
            ...
            with self.tenant_storage.get_video_capture(video_blob_name) as cap:
                ...
                if frame_count_val is None:
                    logger.error(f"Could not read frame count from video: {video_blob_name}")
                    return {"status": StepStatus.ERROR.value, "error": f"Could not read frame count from video: {video_blob_name}"}
                
                total_frames = int(frame_count_val)
                ...
                while True:
                    ...
                    if self.is_stop_requested():
                        ...
                        if upload_tasks:
                            ...
                            # Note: upload_tasks contains futures that are already being handled by the executor
                        
                        # Log crops uploaded before cancellation
                        if all_crop_paths:
                            gcs_urls = [f"gs://{self.tenant_storage.config.bucket_name}/{path}" for path in all_crop_paths]
                    ...
                    try:
                        ...
                        if frame_number == 0 or previous_frame_rgb is None:
                        ...
                        if affine_matrix is None:
                            logger.warning(f"Failed to calculate affine transform for frame {frame_number}")
                            affine_matrix = self.tracker.get_identity_affine_matrix()

                        # Apply the affine transformation to the detections
                        if isinstance(detections, list):
                            if len(detections) > 0:
                                detections = detections[0]  # Take first detection if it's a list
                            else:
                                detections = sv.Detections.empty()
                        ...
                        if isinstance(detections, sv.Detections):
                            ...
                            logger.warning(f"Unexpected detections type at frame {frame_number}: {type(detections)}")
    ...
    async def _upload_crop_batch(self, upload_tasks, batch_counter, video_guid):
        ...
        try:
            ...
            if successful_paths:
                gcs_urls = [f"gs://{self.tenant_storage.config.bucket_name}/{path}" for path in successful_paths]
    ...
    def _is_crop_quality_sufficient(self, crop: np.ndarray) -> bool:
        ...
        contrast = float(np.std(gray_crop.astype(np.float32)))
