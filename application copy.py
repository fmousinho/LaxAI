...
def _initialize_team_identifier(
    video_model: VideoModel, # VideoModel is initialized *before* this function call
    tools: VideoToools, 
    ...
    for frame_idx_to_process in sorted_target_indices: # type: ignore # sorted_target_indices is list[int]
        ...
        # Generate detections and tracks for the sampled frame.
        # We need the transform matrix from generate_tracks to potentially transform ROIs back
        # to the current frame's system if the embedding function needs it.
        detections = video_model.generate_detections(frame, frame_idx=frame_idx_to_process) 
        tracks, transform_matrix = video_model.generate_tracks(frame, detections) # Call generate_tracks to get detections and transform
        if len(detections) >= min_detections_for_sampling_frame: # Check detections count, not tracks
            sampled_frames_data.append((frame, detections, transform_matrix)) # Store frame, detections, and transform
...
def run_application(store: Store, writer: Optional[SummaryWriter] = None, 
    ...
    #--- Main Logic ---
    try: 
        with VideoToools(input_video_path=temp_video_path, output_video_path="results.mp4") as tools:
            ...
            for frame_rgb in main_processing_generator: # frame is RGB
                ...
                tracks, transform_matrix = video_model.generate_tracks(frame_rgb, detections) # Get tracks (stabilized) and transform matrix
                ...
                output_frame_rgb = tools.draw_tracks(frame_rgb, tracks, team_id_getter=video_model.get_smoothed_team_id_from_map, transform_matrix=transform_matrix) # Pass the transform matrix
        ...
        logger.critical("Message: %s", exc_info=True)
